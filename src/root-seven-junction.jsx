...
import {
  ...
  Radar, Route, Send, ShieldCheck, Sparkles, Sprout, TimerReset, X,
} from 'lucide-react';
...
function PromptCopy({ items }) {
  const groups = [];
  let bullets = [];
  const flush = () => { if (bullets.length) { groups.push(<ul key={`list-${groups.length}`}>{bullets.map((item) => <li key={item}>{item}</li>)}</ul>); bullets = []; } };
  items.forEach((item) => {
    if (item.startsWith('- ')) { bullets.push(item.slice(2)); return; }
    flush(); groups.push(<p key={`${groups.length}-${item.slice(0, 24)}`}>{item}</p>);
  });
  flush();
  return <div className="r7-workbook-prompt">{groups}</div>;
}
...
function Opening() {
  ...
  return <section className="r7-opening">
    <div className="r7-opening-copy"><p className="r7-eyebrow"><Route /> Root Seven · The Opportunity Junction</p><h1>Earning Expansion &amp; Income Diversity</h1><blockquote>{rootSevenOpening.coreQuestion}</blockquote></div>
    <div className="r7-opening-story"><div><p className="r7-eyebrow"><Sparkles /> Ivy, Eli &amp; Sage arrive</p><h2>Every route is advertising at once. None gets to hide its full cost.</h2><Paragraphs items={rootSevenOpening.story} /></div><img src="/rootwise-sage-cutout.png" alt="Sage, the RootWise guide" /></div>
    <fieldset className="r7-opening-choice"><legend>When you imagine earning more, which answer appears first?</legend><p>No answer is graded. Root Seven will test what that instinct solves, costs, and leaves untouched.</p><div>{choices.map((choice) => <button type="button" className={answer === choice ? 'is-selected' : ''} onClick={() => setAnswer(choice)} key={choice}>{answer === choice ? <Check /> : <ArrowRight />}{choice}</button>)}</div>{answer && <p className="r7-opening-response">“Keep that answer. Root Seven will help you see what it solves, what it costs, and what it leaves untouched.”</p>}</fieldset>
...
function AdultLevels({ lesson }) {
  return <section className="r7-card r7-levels"><header><p className="r7-eyebrow"><Radar /> Understand · Recognize · Examine</p><h2>Keep the Junction conflict in view while the earning decision becomes visible.</h2></header><div>
    <article><span>01</span><div><p className="r7-level-label">Understand · What is it?</p><h3>The earning structure</h3><ul>{lesson.understand.map((item) => <li key={item}>{item}</li>)}</ul></div></article>
    <article><span>02</span><div><p className="r7-level-label">Recognize · Where does it appear?</p><h3>The route outside Opportunity Junction</h3><ul>{lesson.recognize.map((item) => <li key={item}>{item}</li>)}</ul></div></article>
    <article><span>03</span><div><p className="r7-level-label">Examine · What is driving the choice?</p><h3>Questions that keep choice open</h3><ul>{lesson.examine.map((item) => <li key={item}>{item}</li>)}</ul></div></article>
  </div></section>;
}
...
const scanLayers = [
  ...
  ['capacity', 'Capacity & Dependence', 'What time, energy, permission, relationship, platform, customer, employer, tool, or condition does this route depend on?'],
];

function OpportunityScan({ value = [], onChange }) {
  return <section className="r7-card r7-scan"><p className="r7-eyebrow"><Radar /> Opportunity Scan</p><h2>Run all four lenses before you leave the route.</h2><div>{scanLayers.map(([id, title, body]) => {
  ...
  })}</div>{value.length === 4 && <p className="r7-scan-result"><ShieldCheck /> The complete earning route is visible.</p>}</section>;
}
...
function DecisionPractice({ lesson, answer, scenario, onAnswer, onScenario }) {
  ...
  return <>
    <section className="r7-card r7-check"><p className="r7-eyebrow"><CircleHelp /> Knowledge Check</p><h2>{lesson.check.prompt}</h2><div className="r7-options">{lesson.check.options.map((option) => <button type="button" className={answer === option.id ? 'is-selected' : ''} aria-pressed={answer === option.id} onClick={() => onAnswer(option.id)} key={option.id}><span>{answer === option.id ? <Check /> : <ArrowRight />}</span>{option.label}</button>)}</div>{checked && <div className={checked.isCorrect ? 'r7-feedback is-correct' : 'r7-feedback'} aria-live="polite"><strong>{checked.isCorrect ? 'The complete route is in frame' : 'One part of the route is still hidden'}</strong><p>{checked.isCorrect ? 'This answer keeps value, payment route, real return, capacity, and dependence connected.' : 'Return to Ivy and Eli’s decision and find the answer that includes the advertised gain and the responsibility required to produce it.'}</p></div>}</section>
    <section className="r7-card r7-scenario"><p className="r7-eyebrow"><Route /> Opportunity Decision Drill</p><h2>{lesson.scenario.prompt}</h2><div className="r7-options">{lesson.scenario.options.map((option) => <button type="button" className={scenario === option.id ? 'is-selected' : ''} aria-pressed={scenario === option.id} onClick={() => onScenario(option.id)} key={option.id}><span>{scenario === option.id ? <Check /> : <ArrowRight />}</span>{option.label}</button>)}</div>{scenarioChoice && <div className={scenarioChoice.strength === 'strong' ? 'r7-feedback is-correct' : 'r7-feedback'} aria-live="polite"><strong>{scenarioChoice.strength === 'strong' ? 'This preserves financial decision capacity' : 'The Junction is advertising over the evidence'}</strong><p>{scenarioChoice.feedback}</p></div>}</section>
  </>;
}
...
function PrivatePanel({ lesson, kind, value, onChange }) {
  ...
  return <section className={`r7-card r7-private ${mirror ? 'is-mirror' : ''}`}><div><p className="r7-eyebrow">{mirror ? <Sparkles /> : <BookOpen />} {title}</p>{mirror ? <h2>{lesson.mirror}</h2> : <PromptCopy items={lesson.workbookPrompt} />}<p>{mirror ? 'Ivy and Eli are mirrors, not answers. Notice your own pattern without grading it.' : 'Use approximate values, ranges, fictional examples, or category names. Keep sensitive employer, customer, tax, contract, and identity details out of this workbook.'}</p><label htmlFor={`r7-${kind}-${lesson.key}`}>{mirror ? 'What did the story reveal?' : 'Your private working note'}<textarea id={`r7-${kind}-${lesson.key}`} rows={6} maxLength={5000} value={current.text || ''} onChange={(event) => onChange({ text: event.target.value, status: current.status === 'saved' ? '' : current.status })} placeholder={mirror ? 'A version of this choice appears in my life when…' : 'Build the route map in your own words…'} /></label><div><button type="button" disabled={!String(current.text || '').trim()} onClick={() => onChange({ text: current.text, status: 'saved' })}><CheckCircle2 /> Save on this device</button><button type="button" onClick={() => onChange({ text: current.text || '', status: 'skipped' })}><TimerReset /> Intentionally skip</button></div>{['saved', 'skipped'].includes(current.status) && <p className="r7-private-status"><Check /> {current.status === 'saved' ? 'Saved privately on this device.' : 'Intentionally skipped. You may return at any time.'}</p>}</div><aside><ShieldCheck /><strong>Privacy boundary</strong><p>These entries stay in this browser and are never sent to Ask Sage. Never store Social Security or tax identification numbers, employer or customer names, confidential agreements, tax returns, pay stubs, account numbers, passwords, verification codes, or private legal details here.</p></aside></section>;
}

function Sources({ lesson }) {
  return <section className="r7-card r7-sources"><p className="r7-eyebrow"><FileSearch /> Source Desk</p><h2>Verify the rule behind the earning route</h2><p>Reviewed against primary public authorities on July 29, 2026. Employment rules, tax obligations, worker-classification standards, licensing, market conditions, and agency guidance can change.</p>{lesson.jurisdictionNote && <p className="r7-jurisdiction">Facts, documents, jurisdiction, and current rules matter. This lesson is general education and not a personal employment, legal, tax, licensing, insurance, or worker-classification conclusion.</p>}<ul>{lesson.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.label}<ArrowRight /></a></li>)}</ul></section>;
}
...
function AskSage({ lesson }) {
  ...
  const messages = useMemo(() => threads[lesson.key] || [{ role: 'assistant', content: `We’re at ${lesson.title}. Ask me to separate the value, payment route, real return, capacity, and shared dependence.` }], [threads, lesson]);
  ...
  const send = async (message) => {
    ...
    try {
      const response = await fetch('/api/sage', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ root: 'seven', message: clean, lesson: { number: lesson.displayNumber, title: lesson.title, story: lesson.story.join(' ').slice(0, 1400), understand: lesson.understand.join(' ').slice(0, 1100), recognize: lesson.recognize.join(' ').slice(0, 1100), examine: lesson.examine.join(' ').slice(0, 800) }, history: messages.slice(-9) }), signal: controller.signal });
...
export default function RootSevenJunction({ go, initialLessonKey, onLessonChange }) {
  ...
  const closeNav = () => { setNavOpen(false); window.requestAnimationFrame(() => menuRef.current?.focus()); };
  const select = (index) => { const next = Math.min(Math.max(index, 0), rootSevenLessons.length - 1); setNavOpen(false); if (onLessonChange && next !== activeIndex) { onLessonChange(rootSevenLessons[next].key); return; } setActiveIndex(next); setVisited((items) => items.includes(rootSevenLessons[next].key) ? items : [...items, rootSevenLessons[next].key]); window.scrollTo({ top: 0, behavior: 'smooth' }); };
