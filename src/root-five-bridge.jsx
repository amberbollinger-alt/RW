...
function RichActivity({ type, onInteract }) {
  const [step, setStep] = useState(0);
  const [choice, setChoice] = useState('');
  const [selected, setSelected] = useState({});
  const touch = (action) => { onInteract(); action?.(); };
  if (!type) return null;
  ...
  if (type === 'bridge-contract') {
    const reveals = [['Cash price', '$1,000'], ['Financed amount', '$1,000'], ['Interest rate', '24% fixed'], ['APR', '29.4%'], ['Term', '12 months'], ['Fees', '$45 origination'], ['Total repayment', 'About $1,185'], ['Collateral', 'None in this fictional offer'], ['Missed-payment consequence', 'Possible fee, reporting, and collection under the agreement and law']];
    return frame(<><p>Select from the advertised payment, then reveal one contract layer at a time. You may revise the bridge at any point.</p><div className="r5-offers"><button type="button" className={choice === 'low' ? 'is-selected' : ''} onClick={() => touch(() => setChoice('low'))}><small>Bridge A</small><strong>$98.75 / month</strong></button><button type="button" className={choice === 'clear' ? 'is-selected' : ''} onClick={() => touch(() => setChoice('clear'))}><small>Bridge B</small><strong>$112 / month · terms shown first</strong></button></div><div className="r5-reveals">{reveals.slice(0, step).map(([label, value]) => <span key={label}><small>{label}</small><strong>{value}</strong></span>)}</div><button type="button" className="r5-reveal-button" disabled={step === reveals.length} onClick={() => touch(() => setStep((value) => value + 1))}>{step === reveals.length ? 'Complete contract revealed' : `Reveal ${reveals[step][0]}`} <ChevronRight /></button>{choice && <p className="r5-result">Current choice: <strong>{choice === 'low' ? 'Bridge A' : 'Bridge B'}</strong>. A sound decision is allowed to change as information changes.</p>}</>);
  }
...
function Practice({ lesson, answer, scenario, onAnswer, onScenario, onActivity }) {
  ...
  return <>
    ...
    <RichActivity type={lesson.interactive} onInteract={onActivity} />
...
export default function RootFiveBridge({ go }) {
  const saved = useMemo(() => readProgress(), []); const [activeIndex, setActiveIndex] = useState(saved.activeIndex || 0); const [visited, setVisited] = useState(saved.visited?.length ? saved.visited : [rootFiveLessons[0].key]); const [completed, setCompleted] = useState(saved.completed || []); const [answers, setAnswers] = useState(saved.answers || {}); const [scenarios, setScenarios] = useState(saved.scenarios || {}); const [workbooks, setWorkbooks] = useState(saved.workbooks || {}); const [activities, setActivities] = useState(saved.activities || {}); const [navOpen, setNavOpen] = useState(false); const menuRef = useRef(null); const closeRef = useRef(null);
  ...
  const select = (index) => { const next = Math.min(Math.max(index, 0), rootFiveLessons.length - 1); setActiveIndex(next); setVisited((items) => items.includes(rootFiveLessons[next].key) ? items : [...items, rootFiveLessons[next].key]); setNavOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); };
