...
const groveRoutes = [
  ...
  { key: 'credit', number: '02', label: 'Credit', angle: '-38deg' },
  ...
  return (
    <main className="grove-page">
      ...
      <section className="grove-radial" aria-label="The seven RootWise paths">
        ...
        {groveRoutes.map((route) => (
          <button
            ...
            onClick={() => route.ready && go('roots/one')}
      ...
      <footer className="grove-next">
        <Map size={18} />
        <div><strong>Root One is open</strong><span>The City of Foundations is ready for your first walk.</span></div>
        <button type="button" onClick={() => go('roots/one')}>Enter the city <ArrowRight size={16} /></button>
      </footer>
    </main>
  );
}
